...
$users_query = "SELECT user_id, username, full_name FROM users WHERE role = 'user' ORDER BY username ASC";
$all_users = $conn->query($users_query)->fetchAll();

// If searching, filter users
if (!empty($search)) {
    $search_stmt = $conn->prepare("
        SELECT user_id, username, full_name, email, total_xp, current_level, last_login
        FROM users
