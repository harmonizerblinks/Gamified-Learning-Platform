...
$overall_stats = $conn->query("
    SELECT
        COUNT(DISTINCT uc.user_course_id) as total_enrollments,
        COUNT(DISTINCT CASE WHEN uc.is_completed = 1 THEN uc.user_course_id END) as completed_enrollments,
...
$course_stats = $conn->query("
    SELECT c.course_id, c.course_title, s.subject_name,
           COUNT(DISTINCT uc.user_course_id) as total_enrollments,
           COUNT(DISTINCT CASE WHEN uc.is_completed = 1 THEN uc.user_course_id END) as completed_count,
           AVG(CASE WHEN uc.is_completed = 1 
